...
#
# Public:
#   exec_chroot_mounts_up
#   exec_chroot_mounts_down
...
exec_chroot_mounts_up() {
  ...
  # Target root must be mounted
  if ! findmnt -rn "${TARGET_DIR}" >/dev/null 2>&1; then
    log "[!] chroot_mounts: TARGET_DIR not mounted: ${TARGET_DIR}"
    ui_msg "TARGET_DIR is not mounted:\n${TARGET_DIR}\nLog: ${LOG_FILE}"
    return 1
  fi
...
# Helper for next steps: run command inside chroot with logging
exec_in_chroot() {
  : "${TARGET_DIR:?TARGET_DIR is required}"
  log "[>] chroot: $*"
